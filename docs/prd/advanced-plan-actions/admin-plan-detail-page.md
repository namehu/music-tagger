---
doc_type: page-spec
page_id: admin_plan_detail_advanced_actions
page_name: Plan 详情页
...
- **核心区块划分**：
  ...
  - [计划项列表]：按 item 展示 `move` 的 `fromPath -> toPath`、warning、状态、错误信息。
  - [执行控制区]：`preview`、`confirm`、`execute`、刷新状态、跳转 jobs。
  - [风险提示区]：展示 `move` 预览警告、执行失败摘要和回滚边界说明。
- **页面内从属交互**：
...
| 计划类型 `type` | enum | 必显 | 本模块重点落 `move` | 无 |
| 作用范围 `scope` | array<object> | 必显 | 展示结构化摘要，不直接暴露原始 JSON | 无 |
| 执行参数 `params` | array<object> | 必显 | `move` 时至少展示 `targetDirTemplate` | 无 |
| 创建人 `createdBy` | string | 必显 | 当前仍默认为管理员 | 无 |
...
| 类型 `kind` | enum | 必显 | `move` | 无 |
| 曲目 `track` | string | 必显 | 当前 v1 仍然以 track 为最小执行单位 | 无 |
| 原路径 `fromPath` | string | 必显 | 展示当前曲目源文件路径 | 无 |
| 目标路径 `toPath` | string | 必显 | 展示执行后完整目标路径 | 无 |
...
### 操作 1：执行 `move` preview
- **触发事件**：点击“生成预览”或“刷新预览”按钮。
- **前置校验**：
  - 当前 plan 必须存在
  - 当前状态必须允许 preview，默认允许在 `draft` 阶段重复 preview
  - `targetDirTemplate` 必须可被后端解析
- **流转结果**：调用 `plans.preview`，生成或刷新 `move` 类型 `plan_items` 与 warning 聚合结果。
- **成功结果**：
  - 生成或刷新 `move` 路径 diff
  - 更新预览摘要
  - 在列表中展示路径变化与 warnings
- **失败结果**：
  - 页面显示错误提示
  - 保留上一次成功的 preview 快照
  - 若为阻断性错误，将当前 preview 视为不可确认
...
stateDiagram-v2
  ...
  running --> failed: any terminal item failure
