...
| --- | --- | --- | --- | --- | --- |
| Plans 管理页 | 独立页面 | 创建 draft、筛选和查看计划列表 | 后台导航新增入口 `/admin/plans` | 进入 Plan 详情页 | 需求稿，设计稿 |
| Plan 详情页 | 独立页面 | 查看预览 diff、警告、执行状态并执行关键操作 | 从 Plans 管理页进入 | 返回列表或进入 jobs 观测 | 需求稿，设计稿 |
| 创建 Plan 抽屉 | 页面内抽屉 | 选择操作类型、作用范围、参数模板 | 归属 `admin-plans-page.md` | 创建 draft 后跳转详情页 | 本 PRD 归纳 |
| 确认执行二次确认窗 | 二次确认窗 | 锁定或执行高风险动作前进行确认 | 归属 `admin-plan-detail-page.md` | 触发 confirm / execute | 本 PRD 归纳 |
...
| Python worker | 异步执行 | `plan_execute` job 执行 | 当前 worker 已支持 scan/transcode，需要新增执行器 | 无 |
...
| 实体 | 状态/动作 | 说明 | 来源 |
| --- | --- | --- | --- |
| Plan | draft / confirmed / running / done / failed / cancelled | 需求稿与设计稿都提出的主状态 | 需求稿，设计稿 |
| PlanItem | pending / running / done / failed / skipped | 单项执行状态 | 需求稿 |
| Plan 操作 | create / preview / confirm / execute / get / list items | 模块 v1 最小控制面 | 需求稿，设计稿 |
| Job | `plan_execute` | 用于串接 worker 异步执行 | 需求稿，设计稿 |

## 6. 字段与约束

| 字段 | 约束 | 默认值 | 适用页面 | 来源 |
| --- | --- | --- | --- | --- |
| `type` | v1 限定为 `rename`、`tag_write` | 无 | 列表页创建抽屉 | 设计稿 + 本 PRD 收敛 |
| `scope` | 至少支持按 trackIds / album / artist 建立作用范围 | 无 | 列表页创建抽屉 | 需求稿 |
| `params` | 随 `type` 变化；preview 前可编辑，confirm 后冻结 | 无 | 列表页创建抽屉，详情页 | 设计稿 |
| `warnings` | preview 生成，可为空 | `[]` | 详情页 | 设计稿 |
| `jobKey` | `plan_execute:{planId}` | 自动生成 | 执行链路 | 设计稿 |
