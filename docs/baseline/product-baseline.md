...
- 用户区 `/library` 已支持全文搜索、排序、无限加载虚拟滚动与播放
- 管理区 `/admin/library` 已支持全文搜索、排序、分页表格、编辑过滤与单曲编辑
- 列表展示和搜索当前都以最新编辑值为准
...
- 已支持用户侧底部播放器打开当前队列抽屉，展示当前曲目、Up Next 与当前队列窗口
- 已支持用户侧在当前队列中点击任意项立即播放、移除单首与清空整个队列
- 已支持把当前浏览器内的队列窗口、曲目、模式、进度、音量和曲库播放上下文持久化到 `localStorage`
- 用户区曲库播放队列保存轻量筛选上下文，并通过服务端按需解析当前曲目前后的播放窗口，避免 5000+ 曲库全量写入浏览器状态
