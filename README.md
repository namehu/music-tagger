...
- 顺序 / 随机 / 单曲循环三种全局播放模式
- 用户侧播放器已支持实时进度、可拖动 seek、缓冲条与底部详情抽屉
- 用户侧播放器已支持当前队列抽屉、Up Next、队列内跳播、移除单首与清空队列
- 播放详情已支持 `plain / LRC / 增强 LRC` 歌词显示；LRC 可逐行高亮，增强 LRC 可逐字高亮
- 浏览器本地播放会话恢复：刷新后恢复队列、曲目、模式、进度和音量，并默认暂停
- 个人歌单的创建、重命名、删除、加歌、移歌与顺序点播
- 双层忽略曲目：用户“我的忽略”与管理员“全局忽略”
- 管理员单曲编辑：元数据、歌词、封面先写数据库，再异步回写源文件
  - 封面会先落为音频同目录、同 basename 的 `.jpg/.png` sidecar，再由 worker 异步嵌回音频文件
  - 如果 web / worker 跑在 Docker 里，挂载给两者的音乐目录都必须可写，否则封面保存或 `track_edit_sync` 会失败
- 转码缓存观测、容量治理与策略配置
...
1. 打开 `/sign-in`。如果系统尚未初始化，该页面会先创建首个管理员；初始化完成后再登录。
...
6. 在用户侧底部播放器切换顺序 / 随机 / 单曲循环，并可打开“详情 / 队列”查看 Up Next、移除单首或清空队列；刷新页面会尽量恢复到暂停状态；管理台只保留临时试听条。
